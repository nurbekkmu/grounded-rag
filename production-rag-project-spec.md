...
8. **Cross-encoder reranking can demote paraphrase evidence.** Measured twice on the hardened golden set: MiniLM lowered MRR on paraphrase questions, and on the 19-question re-sweep it pushed one question's evidence out of the top-20 entirely (hybrid failure@20 13%→20% with rerank). A 12x-slower bge-reranker matched, not beat, MiniLM — the weakness is method-level on this question type, not model size.
9. **Behavior-preserving rewrites are provable.** The full readability rewrite (dense expressions → named steps; a ParagraphPacker class replacing nonlocal closures) merged only after byte-identical chunker outputs (which certify every cached context/embedding), 16/16 unit tests, the NLI selftest, digit-identical eval metrics, and a green CI run on the rewritten code.
